...
function doSearch(type, customQuery) {
    ...
    .then(function(data) {
        ...
        // Step 2b: ChEBI (shown alongside PubChem badge in summary; no separate step indicator)
...
function renderResults(data, query) {
    ...
    if (!hasResults) {
        ...
        document.getElementById('noResults').innerHTML =
            ...
            (data.workflow && data.workflow.chebi   && data.workflow.chebi.searched   ? ', ChEBI' : '') +
    ...
    if (wf.chebi   && wf.chebi.searched)   sources.push('ChEBI');
    ...
    // ── ChEBI / PubChem Classification Results ───────────────────────────────
    var chebiHtml = '';
    if (data.chebi && data.chebi.length > 0) {
        chebiHtml = '<div class="card border-0 shadow-sm mb-3">' +
            '<div class="card-header fw-semibold text-white" style="background:#6f42c1">' +
            '<i class="bi bi-diagram-3 me-2"></i>Chemical Classification (' + data.chebi.length + ')' +
            '<span class="badge bg-white ms-2 small" style="color:#6f42c1">Pharmacology &amp; Roles</span>' +
            '</div><div class="row g-3 p-3">';
        data.chebi.forEach(function(c) {
            chebiHtml += '<div class="col-md-4"><div class="card h-100" style="border-color:#6f42c1"><div class="card-body">' +
                '<h6 class="fw-bold mb-1" style="font-size:.85rem">' + escHtml(c.name) + '</h6>' +
                '<div class="small text-muted mb-2"><strong>ID:</strong> ' + escHtml(c.chebi_id) + '</div>' +
                '</div><div class="card-footer bg-white border-0">' +
                '<a href="' + escHtml(c.chebi_url) + '" target="_blank" class="btn btn-sm w-100 text-white" style="background:#6f42c1"><i class="bi bi-box-arrow-up-right me-1"></i>View on PubChem</a>' +
                '</div></div></div>';
        });
        chebiHtml += '</div></div>';
    }
