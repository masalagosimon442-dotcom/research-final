...
class ExternalSearch {
     ...
     * NOTE: ChEBI 2.0 (launched 2025) retired the old REST API endpoints.
     * The old /webservices/rest/search endpoint now returns 404.
     * We fall back to PubChem's classification API which provides similar
     * biological role / ontology data.
     */
    public function searchChEBI(string $query): array {
        ...
        try {
            ...
            // Use PubChem name→CID lookup then fetch pharmacology/classification
            // as a ChEBI substitute providing biological role information
            $cidUrl = "https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/name/{$encodedQuery}/cids/JSON?MaxRecords=1";
            $cidData = $this->httpGet($cidUrl);

            if (!$cidData || empty($cidData['IdentifierList']['CID'])) {
                $this->setCache($cacheKey, 'chebi', []);
                return [];
            }

            $cid = $cidData['IdentifierList']['CID'][0];

            // Fetch classification (biological roles, pharmacology)
            $classUrl = "https://pubchem.ncbi.nlm.nih.gov/rest/pug_view/data/compound/{$cid}/JSON?heading=Pharmacology+and+Biochemistry";
            $classData = $this->httpGet($classUrl);

            $results = [];
            if ($classData && isset($classData['Record']['Section'])) {
                foreach ($classData['Record']['Section'] as $section) {
                    $heading = $section['TOCHeading'] ?? '';
                    if (in_array($heading, ['Pharmacology', 'MeSH Pharmacological Classification', 'ATC Code', 'Drug Classes'])) {
                        $info = $section['Information'][0]['Value']['StringWithMarkup'][0]['String'] ?? null;
                        if ($info) {
                            $results[] = [
                                'source'    => 'PubChem Classification',
                                'chebi_id'  => 'CID:' . $cid,
                                'name'      => $heading . ': ' . mb_substr($info, 0, 120),
                                'chebi_url' => "https://pubchem.ncbi.nlm.nih.gov/compound/{$cid}#section=Pharmacology-and-Biochemistry",
                            ];
                        }
                    }
            ...
            // If no classification data, return a basic entry linking to PubChem
            if (empty($results)) {
                $synUrl  = "https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/cid/{$cid}/property/IUPACName,MolecularFormula/JSON";
                $synData = $this->httpGet($synUrl);
                $iupac   = $synData['PropertyTable']['Properties'][0]['IUPACName'] ?? null;
                $formula = $synData['PropertyTable']['Properties'][0]['MolecularFormula'] ?? null;
                if ($iupac || $formula) {
                    $results[] = [
                        'source'    => 'PubChem Classification',
                        'chebi_id'  => 'CID:' . $cid,
                        'name'      => $iupac ?? $formula,
                        'chebi_url' => "https://pubchem.ncbi.nlm.nih.gov/compound/{$cid}",
                    ];
                }
            }
    ...
    private function httpGet(string $url, bool $raw = false): mixed {
        $ch = curl_init($url);
        // Use XML accept header for raw (XML) requests (e.g. ChEBI, NCBI efetch),
        // JSON accept header for all others.
        $acceptHeader = $raw ? 'Accept: application/xml, text/xml, */*' : 'Accept: application/json';
        curl_setopt_array($ch, [
            ...
            CURLOPT_HTTPHEADER     => [$acceptHeader],
        ]);
